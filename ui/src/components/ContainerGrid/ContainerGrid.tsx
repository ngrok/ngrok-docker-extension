...
import LastConnectionCell from "./components/LastConnectionCell";
...
const ContainerGrid: React.FC = () => {
  ...
  const transformContainerToRow = (container: NgrokContainer): ContainerGridRow => {
    ...
    return {
      ...
      port: container.Port.PublicPort,
      url: runningEndpoint?.url || '',
      trafficPolicy: config?.trafficPolicy ? 'YES' : 'NO',
      lastConnection: 'tbd', // Data source to be determined
  ...
  const columns: DataGridColumnType = [
    ...
    },
    {
      ...
      width: 73,
      renderCell: (params) => (
        <Box sx={{ color: params.row.isOnline ? '#505968' : '#677285', fontWeight: params.row.isOnline ? 400 : 300 }}>
          {params.value}
        </Box>
      )
    },
