...
func (m *manager) convergeAgent(ctx context.Context, config store.AgentConfig) error {
	if config.ExpectedState == AgentStateOffline {
		if err := m.handleAgentOfflineState(); err != nil {
			return err
		}
	}
...
// handleAgentOfflineState manages disconnecting the agent and setting offline status
func (m *manager) handleAgentOfflineState() error {
	// disconnect the agent and set the state to offline
	m.disconnectAgent()
